...
def _build_feature_usage_report(
    ...
    if hasattr(logistic, "coef_"):
        logistic_coef = (
            logistic.coef_[0]
            if getattr(logistic.coef_, "ndim", 1) > 1
            else logistic.coef_
        )
    else:
        logistic_coef = np.zeros(len(feature_names), dtype=float)
    if linear is not None and hasattr(linear, "coef_"):
        try:
            linear_coef = (
                linear.coef_[0]
                if getattr(linear.coef_, "ndim", 1) > 1
                else linear.coef_
            )
        except AttributeError:
            linear_coef = np.full(len(feature_names), np.nan, dtype=float)
    else:
        linear_coef = np.full(len(feature_names), np.nan, dtype=float)
    data = {
    ...
    report["abs_logistic_coef"] = report["logistic_coef"].abs()
    report.sort_values("abs_logistic_coef", ascending=False, inplace=True)
    return report
...
def train_full_models(
    ...
    _log_feature_breakdown(X_full.columns.tolist(), "Full training matrix")
    ...
    report = _build_feature_usage_report(X_full.columns.tolist(), logistic, linear if reg_mask.sum() >= 2 else None)
    report_path = output_dir / "feature_usage.csv"
    report.to_csv(report_path, index=False)
    top_features = report.head(10)
    summary = ", ".join(
        f"{row.feature} ({row.logistic_coef:.3f})" for row in top_features.itertuples()
    )
    logger.info("Top logistic contributors: %s", summary)
