...
2. **Map column names in the config**. Copy `config/example.yaml` and update the `io` section to match your data. Example for the wide file shown in the error transcript:
...
flypca fit-lag-pca \
  ...
  --out outputs/artifacts/models/lagpca.joblib

flypca project \
  --model outputs/artifacts/models/lagpca.joblib \
  --data data/manifest.csv \
  --out outputs/artifacts/projections/

flypca features \
  --data data/manifest.csv \
  --config config/example.yaml \
  --model outputs/artifacts/models/lagpca.joblib \
  --projections outputs/artifacts/projections/ \
  --out outputs/artifacts/features.parquet

flypca cluster \
  --features outputs/artifacts/features.parquet \
  --config config/example.yaml \
  --projections-dir outputs/artifacts/projections/ \
  --method gmm \
  --out outputs/artifacts/cluster.csv \
...
flypca report \
  --features outputs/artifacts/features.parquet \
  --clusters outputs/artifacts/cluster.csv \
  --model outputs/artifacts/models/lagpca.joblib \
  --projections outputs/artifacts/projections/ \
  --out-dir outputs/artifacts/
```

Outputs are written under `outputs/artifacts/` by default: the trained PCA model (`models/`), projected PC trajectories (`projections/`), engineered features (`features.parquet`), clustering assignments, summary figures (`figures/`), and a Markdown report describing variance explained, cluster metrics, and representative trajectories.

CLI entry points (Typer-based):

```bash
flypca fit-lag-pca --data data/manifest.csv --config config/example.yaml --out outputs/artifacts/models/lagpca.joblib
flypca project --model outputs/artifacts/models/lagpca.joblib --data data/manifest.csv --out outputs/artifacts/projections/
flypca features --data data/manifest.csv --config config/example.yaml --model outputs/artifacts/models/lagpca.joblib --projections outputs/artifacts/projections/ --out outputs/artifacts/features.parquet
flypca cluster --features outputs/artifacts/features.parquet --config config/example.yaml --projections-dir outputs/artifacts/projections/ --method gmm --out outputs/artifacts/cluster.csv --label-column reaction
...
flypca cluster \
  ...
  --labels-column-name user_score_odor \
  --out outputs/artifacts/cluster.csv
flypca report --features outputs/artifacts/features.parquet --clusters outputs/artifacts/cluster.csv --model outputs/artifacts/models/lagpca.joblib --projections outputs/artifacts/projections/ --out-dir outputs/artifacts/
```
