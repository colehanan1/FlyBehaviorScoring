...
- Path to NEW (corrected) labels: `data/scoring_results_opto_new_MINIMAL.csv`
...
diff /path/to/OLD_labels.csv data/scoring_results_opto_new_MINIMAL.csv

# Or for better comparison:
python scripts/analysis/compare_labels.py \
  /path/to/OLD_labels.csv \
  data/scoring_results_opto_new_MINIMAL.csv
```

(I can create `scripts/analysis/compare_labels.py` script if you have old backup)
...
flybehavior-response train \
  ...
  --n-pcs 12 \
  --seed 42 \
  --artifacts-dir outputs/artifacts/rf_corrected_labels_v1
```
...
ls -ltr outputs/artifacts/rf_corrected_labels_v1/

# Look at the predictions (replace TIMESTAMP with actual timestamp)
python scripts/analysis/analyze_errors.py outputs/artifacts/rf_corrected_labels_v1/TIMESTAMP/predictions_random_forest_test.csv
```
...
python scripts/analysis/compare_label_versions.py \
  ...
  outputs/artifacts/rf_corrected_labels_v1/TIMESTAMP/metrics.json
```
...
flybehavior-response train \
  ...
  --n-pcs 15 \
  --seed 42 \
  --artifacts-dir outputs/artifacts/rf_tuned_v2
```
...
flybehavior-response train \
  ...
  --cv 5 \
  --seed 42 \
  --artifacts-dir outputs/artifacts/rf_cv_final
```
...
# 2. Check new errors
python scripts/analysis/analyze_errors.py outputs/artifacts/rf_corrected_labels_v1/LATEST_TIMESTAMP/predictions_random_forest_test.csv
...
python scripts/analysis/compare_labels.py OLD_labels.csv NEW_labels.csv
...
# 4. Compare metrics
python scripts/analysis/compare_label_versions.py OLD_metrics.json NEW_metrics.json
...
# ANALYZE ERRORS
python scripts/analysis/analyze_errors.py outputs/artifacts/rf_corrected_labels_v1/*/predictions_random_forest_test.csv
