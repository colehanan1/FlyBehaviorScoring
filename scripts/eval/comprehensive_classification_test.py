#!/usr/bin/env python3
"""Comprehensive test of all models across all classification modes."""
...
from _paths import data_dir, outputs_dir, resolve_path  # noqa: E402

# Run all models with all three classification modes
data_csv = str(resolve_path(data_dir() / "all_envelope_rows_wide.csv"))
labels_csv = str(resolve_path(data_dir() / "scoring_results_opto_new_MINIMAL.csv"))
results_root = outputs_dir() / "comprehensive_test"
...
print("="*120)
print("COMPREHENSIVE CLASSIFICATION MODE TEST")
print("="*120)
print(f"\nConfiguration:")
...
for mode_key, mode_name in classification_modes.items():
    ...
    artifacts_dir = results_root / mode_key
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            results_summary[mode_key] = {"status": "FAILED", "error": result.stderr}
            continue

        print(f"✓ {mode_name} completed successfully!")

        # Load and parse results
        latest_dir = sorted(artifacts_dir.glob("20*"))[-1]
